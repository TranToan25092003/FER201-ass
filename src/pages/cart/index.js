...
function Cart() {
   ...
   * ====================================
   */
  const { cart, setCart, cartCount, setCartCount, userAccount } =
    useCartFetch();
  ...
  const calculateTotal = () => {
    const totalPriceBeforeVAT = cart.reduce(
  ...
  const handleSubmit = async (e) => {
    ...
    if (cart.length === 0) {
      toast.warn("Cannot submit empty cart", {
    ...
    try {
      ...
      if (insufficientStockItems.length > 0) {
        const itemNames = insufficientStockItems
          ...
          .join(", ");
        toast.error(
          `The following items have insufficient stock: ${itemNames}`,
          { position: toast.POSITION.TOP_CENTER }
        );
        return;
      }
